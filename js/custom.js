...
//  12. Three.js Interactive Cube <--- NEW
//
// ------------------------------------------------
...
// --------------------------------------------- //
// 12. Three.js Interactive Cube Start
// --------------------------------------------- //
// 使用 'DOMContentLoaded' 確保在執行 three.js 代碼前，所有 HTML 元素都已加載
document.addEventListener('DOMContentLoaded', function() {

    // 檢查 three.js 和 canvas 元素是否存在
    ...
    function animate() {
        ...
        // 5. & 7. 結合基礎旋轉速度和鼠標位置影響，更新立方體旋轉
        // 鼠標Y軸位置（從-1到1）影響X軸的旋轉速度
        // 鼠標X軸位置（從-1到1）影響Y軸的旋轉速度
        const rotationX_speed = 0.005 + (mouseY * 0.02);
        const rotationY_speed = 0.005 + (mouseX * 0.02);

        cube.rotation.x += rotationX_speed;
        cube.rotation.y += rotationY_speed;
    ...
    function onMouseMove(event) {
        // 7. 將鼠標位置從像素轉換為 -1 到 1 的範圍
        mouseX = (event.clientX / window.innerWidth) * 2 - 1;
        mouseY = -(event.clientY / window.innerHeight) * 2 + 1; // Y軸需要反轉
    }
    window.addEventListener('mousemove', onMouseMove, false);

    // 8. 確保代碼在窗口大小改變時能夠自適應更新
